                    ...
                    .map(([floor, floorZones]) => (
                      <div key={floor} className="border-2 border-line bg-white">
                        ...
                        <div className="divide-y divide-line">
                          {floorZones.map((z) => (
                            <div key={z.zone_id} className="flex items-center justify-between px-4 py-2.5">
                              <div>
                                <p className="font-mono text-xs text-ink-600">{z.zone_id}</p>
...
function QuickAddZonesDrawer({ location, onClose, onAdded }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const specs = STANDARD_ROOMS.filter((r) => checked[r.key]).map((r) => ({
      zone_type: r.zone_type,
      suffix: r.suffix
    }));
    if (specs.length === 0) return toast.error('Select at least one room to add.');
        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            {STANDARD_ROOMS.map((r) => (
              <label key={r.key} className="flex items-center gap-3 border-2 border-line bg-white px-3.5 py-2.5">
                <input
                  type="checkbox"
