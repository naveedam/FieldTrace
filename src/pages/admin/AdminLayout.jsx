...
export default function AdminLayout() {
  ...
  async function checkOrg() {
    const { data: orgId, error: rpcError } = await supabase.rpc('ft_current_org_id');
  ...
  if (orgName === null) {
    return <SetupOrganization onLinked={checkOrg} />;
  }
      ...
      <header className="flex items-center justify-between border-b-2 border-ink bg-ink px-6 py-4 text-paper">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-line">{orgName}</span>
          </div>
        </div>

        <button
          onClick={() => supabase.auth.signOut()}
          className="flex items-center gap-1.5 text-sm text-line hover:text-paper"
        >
          <LogOut size={15} />
          Sign out
        </button>
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-2.5 border-l-2 px-3 py-2 text-sm font-semibold ${
                          isActive
                            ...
                            : 'border-transparent text-ink-600 hover:bg-paper'
                        }`
                      }
                    >
                      <Icon size={15} />
